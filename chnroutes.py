...
def generate_redsocks(metric):
    results=fetch_ip_data()
    
    upscript_header=textwrap.dedent("""\
    iptables -t nat -N REDSOCKS
    iptables -t nat -A REDSOCKS -d #SHADOWSOCKS_SERVER_ADDRESS#/32 -j RETURN
    ...
    upfile=open('addrules','w')
    downfile=open('delrules','w')
    ...
    for ip,_,mask in results:
        upfile.write('iptables -t nat -A REDSOCKS -d %s/%s -j RETURN\n'%(ip,mask))
    
    upfile.close()
    downfile.close()
    
    print "For Shadowsocks with Redsocks"
...
if __name__=='__main__':
    ...
    parser.add_argument('-p','--platform',
                        ...
                        default='openvpn',
                        nargs='?',
                        help="Target platforms, it can be openvpn, redsocks, linux," 
    ...
    elif args.platform.lower() == 'redsocks':
        generate_redsocks(args.metric)
